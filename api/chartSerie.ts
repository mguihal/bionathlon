...
import {
  Sampling,
  SerieModifier,
  ChartSerieQueryParams,
  ChartSerieResponse,
  chartSerieQueryParamsSchema,
...
function formatDateWeek(date: Date) {
  ...
  return `${date.getFullYear()}-${week < 10 ? `0${week}` : week}`;
}
    ...
    handler: async (res, _, query: ChartSerieQueryParams) => {
      return withDb(async (db) => {
        ...
        if (query.sampling === 'none') {
          ...
          const minScore = -33;
          const maxScore = 66;
          sampledData = Array(maxScore - minScore + 1)
            .fill(0)
            .map((e, i) => `${i + minScore}`);
        }
        ...
        points = sampledData.map((sample, si) => {
          let filteredGames: EnrichedGame[] = enrichedGames.filter((game) => {
            return (
              filterGames(game, sample, query.sampling, query.modifier) &&
          ...
          if (query.sampling === 'playedSession') {
            ...
            filteredGames = filteredGames.filter((g, i) =>
              query.modifier === 'cumulated' ? i <= nbSample : i === nbSample,
            );
            ...
            switch (query.type) {
              ...
              case 'nbRondelles':
                return {
                  key: sample,
                  shouldDisplay: true,
                  value: filteredGames.reduce((acc, cur) => acc + sampleToAttribute(cur), 0),
                };
              case 'nbMatchs':
                return {
                  key: sample,
                  shouldDisplay: true,
                  value: filteredGames.filter((g) => sampleToAttribute(g) > 0).length,
                };
              case 'avgPoints':
                return {
                  key: sample,
                  shouldDisplay: true,
                  value: filteredGames.reduce((acc, cur) => acc + sampleToAttribute(cur), 0) / filteredGames.length,
                };
              case 'topScore':
                return {
                  key: sample,
                  shouldDisplay: true,
                  value: Math.max(...filteredGames.map((g) => sampleToAttribute(g))),
                };
              case 'worstScore':
                return {
                  key: sample,
                  shouldDisplay: true,
                  value: Math.min(...filteredGames.map((g) => sampleToAttribute(g))),
                };
              case 'nbPlayers':
                return {
                  key: sample,
                  shouldDisplay: true,
                  value: filteredGames
                    .filter((g) => sampleToAttribute(g) > 0)
                    .map((game) => game.playerId)
                    .filter((p, i, a) => a.indexOf(p) === i).length,
                };
              default:
          ...
          if (query.type === 'nbPoints') {
            ...
            const filteredGamesWithoutPlayerFilter = enrichedGames.filter((game) =>
              filterGames(game, sample, query.sampling, query.modifier),
            );
            ...
            return {
              key: sample,
              value:
                query.playerFilter === 'all'
                  ? filteredGames.length
                  : winnerPlayerIds.reduce<number>((acc, cur) => acc + (cur === Number(query.playerFilter) ? 1 : 0), 0),
              shouldDisplay: true,
            };
            ...
            const sessionGames = groupByDateTime(filteredGamesWithoutPlayerFilter);
            const sessionsWithSuddenDeath = Object.keys(sessionGames).map((session) =>
              getSuddenDeathGames(sessionGames[session], false),
            );
            ...
            return {
              ...
              value:
                query.playerFilter === 'all'
                  ...
                  : sessionsWithSuddenDeath.reduce(
                      (acc, cur) =>
                        acc + (cur.filter((g) => g.playerId === Number(query.playerFilter)).length > 0 ? 1 : 0),
                      0,
                    ),
              shouldDisplay: true,
            };
            ...
            return {
              ...
              value:
                query.playerFilter === 'all'
                  ...
                  : sessionsWithSuddenDeath.reduce(
                      (acc, cur, i) =>
                        acc + (cur.length > 0 && winnerPlayerIds[i] === Number(query.playerFilter) ? 1 : 0),
                      0,
                    ),
              shouldDisplay: true,
            };
            ...
            return {
              key: sample,
              value: filteredGames.reduce(
                (acc, cur) =>
                  acc + Math.min(cur.scoreLeftBottle || 0, cur.scoreMiddleBottle || 0, cur.scoreRightBottle || 0),
                0,
              ),
              shouldDisplay: true,
            };
        ...
        if (query.modifier === 'smooth') {
          points = points.map((point, index, arr) => {
            const smoothOn = arr
              ...
              .slice(0, 10);
            const smoothedValue =
              smoothOn.length > 0 ? smoothOn.reduce((prev, cur) => prev + cur.value, 0) / smoothOn.length : point.value;
          ...
          points = points.map((point) => {
            if (smoothOn.length === 0) {
              ...
              return {
                ...point,
                value: (smoothOn[smoothOn.length / 2 - 1] + smoothOn[smoothOn.length / 2]) / 2,
              };
            } else {
