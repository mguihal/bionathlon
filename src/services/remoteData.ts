...
export const dataSuccess = <T>(data: T): DataFetchedSuccess<T> => ({ _tag: 'dataFetchedSuccess', data });
export const dataError = <E>(error: E): DataFetchedError<E> => ({ _tag: 'dataFetchedError', error });

const fold = <T, E>(data: DataResponse<T, E>) => <R>(
  success: (value: T) => R,
  error: (error: E) => R,
  notFetched: () => R,
  pending?: () => R
...
export const wrap = <T, E>(data: DataResponse<T, E>): WrapData<T, E> => {
  return {
    fold: fold(data),
    getOrElse: <R>(other: R) => fold(data)<T | R>((p) => p, () => other, () => other, () => other),
